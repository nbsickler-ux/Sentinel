...
const MIGRATIONS = [
  ...
  {
    version: 3,
    name: "add_status_code_to_request_log",
    up: `
      ALTER TABLE request_log ADD COLUMN IF NOT EXISTS status_code INTEGER;
...
export function logRequest({ callerWallet, endpoint, method, chainId, isContract, paymentAmount, paymentCurrency, verdict, responseTimeMs, statusCode }) {
  if (!pool || !dbReady) return; // Silently skip if no DB

  // Fire-and-forget — never awaited by the caller
  pool.query(
    `INSERT INTO request_log
       (caller_wallet, endpoint, method, chain_id, is_contract, payment_amount, payment_currency, verdict, response_time_ms, status_code)
  ...
  const [summary, topEndpoints, topCallers, overTime, recent] = await Promise.all([
    ...
    pool.query(`
      SELECT endpoint, method, COUNT(*)::int AS count,
        COUNT(*) FILTER (WHERE status_code = 200)::int AS success_count,
    ...
    `),
    pool.query(`
      SELECT caller_wallet, endpoint, method, status_code, timestamp, verdict, response_time_ms
      FROM request_log
