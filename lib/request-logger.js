...
export function createRequestLogger({ alchemyApiKey, network }) {
  ...
  return function requestLoggerMiddleware(req, res, next) {
    ...
    // Capture attribution signals — helps trace which post / directory / crawler drove the hit
    const referrer = req.headers["referer"] || req.headers["referrer"] || null;
    const userAgent = req.headers["user-agent"] || null;

    // Detect whether this request was actually paid via x402 or bypassed
    const hasX402Payment = !!req.headers["x-payer-address"];
    const wasBypassed = !!req.headers["x-bypass-secret"];
    ...
    // For paid endpoints, intercept res.json() to capture the verdict and status code
    ...
    res.json = function (body) {
      ...
      // Only count revenue for actual x402 payments — bypass/test calls get null
      const actuallyPaid = hasX402Payment && !wasBypassed;
      ...
      checkIsContract(callerWallet, alchemyApiKey).then((isContract) => {
        logRequest({
          ...
          chainId,
          isContract,
          paymentAmount: actuallyPaid ? (ENDPOINT_PRICE[endpoint] || null) : null,
          paymentCurrency: actuallyPaid ? "USDC" : null,
          // Truncate to 500 chars so the full helpful error (incl. example) survives in admin reports.
          // 50 chars was hiding "Missing required field 'address'. Send {\"address\":" with no remediation hint.
          verdict: typeof verdict === "string" ? verdict.substring(0, 500) : null,
          responseTimeMs,
          statusCode,
          referrer,
          userAgent,
